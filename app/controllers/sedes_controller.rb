class SedesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @sede.save
        format.html { redirect_to sede_url(@sede), notice: "Sede was successfully created." }
  ...
  def update
    respond_to do |format|
      if @sede.update(sede_params)
        format.html { redirect_to sede_url(@sede), notice: "Sede was successfully updated." }
  ...
  private
    ...
    def sede_params
      params.require(:sede).permit(:nombre, :localidad)
    end
end
